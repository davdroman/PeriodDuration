...
final class ISO8601Tests: XCTestCase {
    func testPeriodDurationScenarios() throws {
        for s in scenarios {
            XCTAssertEqual(PeriodDuration(iso8601: s.input), s.output.map(PeriodDuration.init))
    ...
    func testPeriodScenarios() throws {
        for s in scenarios {
            XCTAssertEqual(Period(iso8601: s.input), s.output.map(Period.init))
    ...
    func testDurationScenarios() throws {
        for s in scenarios {
            XCTAssertEqual(Duration(iso8601: s.input), s.output.map(Duration.init))
